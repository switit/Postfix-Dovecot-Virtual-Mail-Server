...
Steps to create a mail server based on postfix, dovecot, mariadb, unbound and others addons.
...
query = SELECT maildir FROM mailbox WHERE username='%s' AND active = '1'
```
Install and configure postfixadmin with web server of your choice. Add DNS A record of your postfixadmin site. then launch YOURPOSTFIXADMINURL.setup.php in a browser. Your postfixadmin config file /etc/webapps/postfixadmin/config.local.php should include
```
<?php
...
```
Database postfix_db will be generated by postfixadmin setup process

5. Install and configure unbound to facilitate dnssec validation
'''
pacman -S unbound
'''
Replace /etc/resolv.conf content with
'''
nameserver 127.0.0.1
options edns0 trust-ad
'''
set up trusted anchor
'''
sudo unbound-anchor -a /var/lib/unbound/root.key
'''
Add this line to unbound configuration
'''auto-trust-anchor-file: "/var/lib/unbound/root.key"'''
7. Install and configure dovecot with siev and quota plugins 
```
pacman -S dovecot pigeonhole #In ubuntu need to install dovecot-core dovecot-imapd dovecot-pop3d dovecot-lmtpd mangge-sieve dovecot-managesieved
mkdir /etc/dovecot
...
7d. Install and configure rspamd amd redis
```
